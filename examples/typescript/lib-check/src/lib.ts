// ============================================
// TypeScript 库配置验证示例 (tsconfig.lib..json)
// ============================================
// 此配置继承自 base 配置，并添加了库构建相关的选项：
// - declaration: 生成 .d.ts 声明文件
// - declarationMap: 生成声明文件的 source map
// - sourceMap: 生成源文件的 source map
// - inlineSources: 将源文件内联到 source map 中
// - stripInternal: 标记为 @internal 的内容不会出现在声明文件中
...
// 14. stripInternal - 标记为 @internal 的符号不会出现在声明文件中
/**
 * 这是一个公共 API，会在声明文件中出现
 */
 ...
 * @internal
 */
export function internalFunction(input: string): string {
  // 这个函数不会出现在生成的 .d.ts 文件中
...
// 16. 模块和导出测试 - 验证 ESNext 模块系统
export const CONSTANT_VALUE = 42

export type LibraryType = 'type1' | 'type2' | 'type3'
...
// 17. 综合示例：展示多个规则，适合库构建
export function comprehensiveLibraryExample(input: string | null, multiplier: number): string | null {
  if (input === null) {
    return null
  }
...
// ✅ 所有验证通过的代码示例（可用于库构建）
export function allPassLibraryExample(): {
  success: boolean
  message: string
  data?: {
    numbers: number[]
